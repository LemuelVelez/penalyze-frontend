...
export default function ManualAttendancePage() {
  ...
  const filteredRecords = useMemo(() => {
    ...
    return records.filter((record) => {
      ...
      return String(record.college ?? "").trim().toLowerCase() === targetCollege;
    });
  ...
  async function loadPageData(nextSchoolYearId = selectedSchoolYearId) {
    ...
    try {
      ...
      toast.error(error instanceof Error ? error.message : "Unable to load manual attendance.");
    } finally {
      setIsLoading(false);
    }
  }
  ...
  function handleFieldChange(field: keyof ManualAttendanceFormState, value: string) {
    setForm((current) => ({
      ...current,
      [field]: value,
    }));
          ...
          <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
            <div>
              ...
              <p className="mt-3 max-w-3xl text-sm leading-6 text-muted-foreground">
                Manual attendance is stored in its own table and uses existing events. The main attendance page stays focused on uploaded files only.
              </p>
            </div>
            ...
            <div className="grid w-full gap-3 sm:grid-cols-2 lg:w-auto">
              ...
              <Select value={collegeFilter || "__all_colleges__"} onValueChange={(value) => setCollegeFilter(value === "__all_colleges__" ? "" : value)}>
                <SelectTrigger className="min-h-12 rounded-2xl lg:w-72">
        ...
        <section className="rounded-3xl border bg-card p-5 shadow-sm">
          ...
          <form onSubmit={handleSubmit} className="mt-5 grid gap-4 lg:grid-cols-4">
            ...
            <label className="space-y-2 lg:col-span-2">
              ...
              <Select value={form.eventId} onValueChange={(value) => handleFieldChange("eventId", value)}>
                <SelectTrigger className="min-h-12 rounded-2xl">
            ...
            <label className="space-y-2">
              ...
              <Input
                ...
                onChange={(event) => handleFieldChange("scannedAt", event.target.value)}
                ...
                {filteredRecords.length ? (
                  filteredRecords.map((record) => (
                    <tr key={record.id} className="border-t">
                      <td className="px-4 py-3 font-black">{record.student_id}</td>
                      <td className="px-4 py-3 font-semibold">{record.name}</td>
                      <td className="px-4 py-3 text-muted-foreground">{record.event_name || "—"}</td>
                      <td className="px-4 py-3 text-muted-foreground">{record.college || "—"}</td>
                      <td className="px-4 py-3 text-muted-foreground">{record.program || "—"}</td>
                      <td className="px-4 py-3">
                        <span className="rounded-full border bg-muted px-3 py-1 text-xs font-black">
                          {record.attendance_type === "zero_attendance" ? "Zero attendance" : "Manual"}
                        </span>
                      </td>
                      <td className="px-4 py-3 text-muted-foreground">{formatDateTime(record.scanned_at ?? record.created_at)}</td>
                    </tr>
                  ))
                ) : (
                  <tr>
